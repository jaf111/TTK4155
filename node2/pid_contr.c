...
int16_t pid_controller(int16_t setpoint, int16_t motor_encoder_max) {	//Calculate the corrected output. This function must be called according the period T
	if(int_tim8 == 1) {
		int_tim8 = 0;

		int16_t measured_value = -motor_read_encoder();
		setpoint = setpoint*(motor_encoder_max/255);

		error = setpoint - measured_value;
		error /= 200;	//36 = 1/(motor_encoder_max/255)
		if ((error > 10) || (error < -10)) {		//If the error is too big, integral (accumulation) value must be restarted
			integral = 0;
		}
		...
		PID_output = Kp*error + (T/1000)*Ki*integral + Kd*derivative/(T/1000);	//Derivative part should not be used with coils (like motors) as increases the noise...
