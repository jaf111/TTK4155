...
void CAN_send(packet* message) {	//Everything is done only through Buffer 0
	...
	*/
	...
	MCP2515_bit_modify(MCP_TXB0SIDH, 0b11111111, message->id>>3);
	MCP2515_bit_modify(MCP_TXB0SIDL, 0b11100000, message->id<<5);
	MCP2515_bit_modify(MCP_TXB0DLC, 0x0F, message->length);
	fprintf(UART_p, " CAN_id: %04X \n\r", (MCP2515_read(MCP_TXB0SIDH)<<3) + (MCP2515_read(MCP_TXB0SIDL)>>5));

	for (uint8_t i = 0; i < MAX_CAN_DATA; i++) {
		//CANdata = message->data[i];
		if (i >= message->length) {message->data[i] = 0x00;}		//If message shorter than maximum, rest is cleared
		MCP2515_bit_modify(MCP_TXB0D0 + i, 0xFF, message->data[i]);
		//fprintf(UART_p, "MCP_TXB0D %4x: %4x \r\n", i, MCP2515_read(MCP_TXB0D0+i));
	}
	MCP2515_request_to_send(MCP_RTS_TX0);
	//fprintf(UART_p, " data 8: %04X \n\r", CANdata[7]);
	/*_delay_ms(10);
